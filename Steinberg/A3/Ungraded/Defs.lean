...
theorem weak_define_of_present_ungraded (R : Type TR) [Ring R] :
  ∀ {g : ChevalleyGenerator A3PosRoot R}, g.ζ ∈ weakA3System.presentRoots → weak_define_ungraded R g = FreeGroup.of g := by
...
def weakA3GradedUngraded (R : Type TR) [Ring R] := PartialChevalleyGroup.mk
  weakA3System
...
scoped notation (priority:=high) "⸨" ζ ", " t "⸩" =>
  (weakA3GradedUngraded R).project {ζ, t}

-- Instantiate macros for ungraded case

macro "declare_A3_ungraded_lin_id_inv_thms" R:term:arg root:term:arg : command =>
  `(command| declare_ungraded_lin_id_inv_thms weakA3GradedUngraded $R $root)

macro "declare_A3_ungraded_trivial_span_expr_thm" R:term:arg r₁:term:arg r₂:term:arg : command =>
  `(command| declare_ungraded_trivial_span_expr_thm weakA3GradedUngraded $R $r₁ $r₂)

macro "declare_A3_ungraded_trivial_span_of_root_pair_thms" R:term:arg r₁:term:arg r₂:term:arg : command =>
  `(command| declare_ungraded_trivial_span_of_root_pair_thms weakA3GradedUngraded $R $r₁ $r₂)

macro "declare_A3_ungraded_single_span_expr_thms" R:term:arg r₁:term:arg r₂:term:arg r₃:term:arg isNeg:num n:num : command =>
  `(command| declare_ungraded_single_span_expr_thms weakA3GradedUngraded $R $r₁ $r₂ $r₃ $isNeg $n)

macro "declare_A3_ungraded_single_span_of_root_pair_thms" R:term:arg r₁:term:arg r₂:term:arg r₃:term:arg isNeg:num n:num : command =>
  `(command| declare_ungraded_single_span_of_root_pair_thms weakA3GradedUngraded $R $r₁ $r₂ $r₃ $isNeg $n)
