...
open PosRootSys WeakChevalleyB3Small
open Set
...
private theorem refl_deg_of_rels_of_single_commutator_of_root_pair
  (ζ η θ : Φ) (C : R) (h_height : height θ = height ζ + height η)
    : ∀ r ∈ rels_of_single_commutator_of_root_pair ⟨ ζ, η, θ, C, h_height ⟩,
      FreeGroup.map refl_deg_of_gen r ∈ rels_of_single_commutator_of_root_pair ⟨ ζ, η, θ, C, h_height ⟩ := by
  intro r h
...
private def pres_of_refl_deg_of_gen (w : WeakChevalleyB3Small Φ R) (g : GradedGen Φ R) : w.group :=
  w.pres_mk (GradedGen.free_mk (refl_deg_of_gen g))

def refl_valid (w : WeakChevalleyB3Small Φ R) :=
...
theorem reflect_degree_of_rels {w : WeakChevalleyB3Small Φ R} (h' : refl_valid w) :
...
def refl_symm {w : WeakChevalleyB3Small Φ R} (h : refl_valid w) : WeakChevalleyB3Small.group w →* WeakChevalleyB3Small.group w :=
...
theorem refl_im (ζ : Φ) (i : ℕ) (hi : i ≤ height ζ) (t : R) {w : WeakChevalleyB3Small Φ R} (h :  refl_valid w) :
