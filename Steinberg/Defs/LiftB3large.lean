...
def rels_of_hom_lift_of_doub_of_αβψ :=
  {
    ...
    (free_mk_mk αβ i hi (t * u))⁻¹ *
    (free_mk_mk ψ k hk (-v))⁻¹
    | (i : ℕ) (k : ℕ) (hi : i ≤ αβ.height) (hk : k ≤ ψ.height) (t : R) (u : R) (v : R)
  }
...
theorem commutator_of_ψ_βψ : single_commutator_of_root_pair weakB3Large.pres_mk ψ βψ β2ψ (2 : R) (by rfl) :=
  weakB3Large.single_commutator_helper ψ βψ β2ψ (2 : R) (by rfl) (by rw [weakB3Large, trivial_commutator_pairs]; simp)

-- Double: {(β, ψ)}
theorem commutator_of_β_ψ : double_commutator_of_root_pair (R := R) weakB3Large.pres_mk β ψ βψ β2ψ (1 : R) (1 : R) (by rfl) (by rfl) :=
...
theorem raw_hom_lift_of_doub_of_αβψ :
  ∀ ⦃i k : ℕ⦄ (hi : i ≤ αβ.height) (hk : k ≤ ψ.height) (t u v : R),
...
theorem b3large_valid : ReflDeg.refl_valid (R := R) weakB3Large :=
...
theorem expr_α_αβ_as_αβ_α :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ α.height) (hj : j ≤ αβ.height) (t u : R),
  {α, i, t} * {αβ, j, u} = {αβ, j, u} * {α, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_α_αβ _ _ t u)]

@[group_reassoc]
theorem expr_β_αβ_as_αβ_β :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ β.height) (hj : j ≤ αβ.height) (t u : R),
  {β, i, t} * {αβ, j, u} = {αβ, j, u} * {β, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_β_αβ _ _ t u)]

@[group_reassoc]
theorem expr_α_ψ_as_ψ_α :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ α.height) (hj : j ≤ ψ.height) (t u : R),
  {α, i, t} * {ψ, j, u} = {ψ, j, u} * {α, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_α_ψ _ _ t u)]

@[group_reassoc]
theorem expr_β_βψ_as_βψ_β :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ β.height) (hj : j ≤ βψ.height) (t u : R),
  {β, i, t} * {βψ, j, u} = {βψ, j, u} * {β, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_β_βψ _ _ t u)]

@[group_reassoc]
theorem expr_β_β2ψ_as_β2ψ_β :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ β.height) (hj : j ≤ β2ψ.height) (t u : R),
  {β, i, t} * {β2ψ, j, u} = {β2ψ, j, u} * {β, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_β_β2ψ _ _ t u)]

@[group_reassoc]
theorem expr_ψ_β2ψ_as_β2ψ_ψ :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ ψ.height) (hj : j ≤ β2ψ.height) (t u : R),
  {ψ, i, t} * {β2ψ, j, u} = {β2ψ, j, u} * {ψ, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_ψ_β2ψ _ _ t u)]

@[group_reassoc]
theorem expr_βψ_β2ψ_as_β2ψ_βψ :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ βψ.height) (hj : j ≤ β2ψ.height) (t u : R),
  {βψ, i, t} * {β2ψ, j, u} = {β2ψ, j, u} * {βψ, i, t} := by
  intro i j hi hj t u
  rw [triv_comm_iff_commutes.1 (comm_of_βψ_β2ψ _ _ t u)]
...
theorem expand_βψ_as_ψ_β_ψ_β_ψ :
  ...
  intro i j hi hj t u
  have := calc
    ⁅{β, j, u}, {ψ, i, t}⁆ = {βψ, i + j, t * u} * {β2ψ, j + 2 * i, u * t^2} := by rw [commutator_of_β_ψ (R := R)]; group
    _ = {β2ψ, j + 2 * i, u * t^2} * {βψ, i + j, t * u} := by rw [comm_left, comm_of_βψ_β2ψ (R := R)]; group
    _ = ⁅{ψ, i, t / 2}, {βψ, i + j, t * u}⁆ * {βψ, i + j, t * u} := by rw [commutator_of_ψ_βψ]; field_simp; group
...
theorem expand_β2ψ_as_ψ_βψ_ψ_βψ :
  ...
  intro i j hi hj t u
  calc
    {β2ψ, i + j, 2 * t * u} = ⁅{ψ, i, t}, {βψ, j, u}⁆ := by rw [commutator_of_ψ_βψ, mul_assoc]
...
theorem expr_ψ_β_as_β_ψ_βψ_β2ψ :
  ...
  calc
    ...
    _ = {β, i, t} * {ψ, j, u} * ⁅{β, i, -t}, {ψ, j, -u}⁆⁻¹ := by grw [inv_of_β, inv_of_ψ]
...
theorem expr_ψ_β_as_β2ψ_βψ_β_ψ :
  ...
  intro i j hi hj t u
  have h₁ : ⁅{β, i, t}, {ψ, j, u}⁆ = {βψ, i + j, t * u} * {β2ψ, i + 2 * j, t * u^2} := by rw [commutator_of_β_ψ (R := R)]; group
...
theorem expr_ψ_βψ_as_βψ_β2ψ :
  ...
  intro i j hi hj t u
  calc
    {ψ, i, t} * {βψ, j, u} = {β2ψ, i + j, 2 * (t * u)} * {βψ, j, u} * {ψ, i, t} := reorder_left_of_eq_comm (commutator_of_ψ_βψ (R := R) hi hj t u)
...
private lemma comm_of_αβ_βψ_20 : ∀ (t u : R), ⁅ {αβ, 2, t}, {βψ, 0, u} ⁆ = 1 := by
  intro t u
...
private lemma comm_of_αβ_βψ_02 : ∀ (t u : R), ⁅ {αβ, 0, t}, {βψ, 2, u} ⁆ = 1 := by
  intro t u
...
private lemma image_of_homog_lift_of_comm_of_αβ_βψ {i j : ℕ} (hi : i ≤ αβ.height) (hj : j ≤ βψ.height)
    : ((i, j) ∈ ij_jk_image) → ∀ (t u : R), ⁅ {αβ, i, t}, {βψ, j, u} ⁆ = 1 := by
...
theorem expr_αβ_βψ_as_βψ_αβ :
  ∀ ⦃i j : ℕ⦄ (hi : i ≤ αβ.height) (hj : j ≤ βψ.height) (t u : R),
...
theorem trivial_comm_of_α_αβψ :
  ...
  grw [←one_mul u, expand_αβψ_as_ψ_αβ_ψ_αβ_ψ hj₁ hj₂, expr_α_ψ_as_ψ_α hi hj₂,
...
private lemma inv_doub_of_αβψ_a :
  ...
  have : (-(1 : R) / 2) = -((1 : R) / 2) := by ring
  grw [←mul_one t, expand_αβψ_as_ψ_αβ_ψ_αβ_ψ hi₁ hi₂, ←neg_mul, neg_mul_comm, expand_αβψ_as_ψ_αβ_ψ_αβ_ψ hi₁ hi₂,
  neg_neg, this, inv_of_ψ hi₂ ((1 : R) / 2), inv_mul_cancel_right, inv_of_αβ,
  inv_mul_cancel_right, inv_of_ψ]
  simp only [one_div, mul_inv_cancel_right, inv_mul_cancel]
...
theorem doub_of_αβψ :
  ...
  intros i hi t
  rcases decompose αβ.height ψ.height i hi with ⟨ i₁, i₂, ⟨ rfl, hi₁, hi₂ ⟩ ⟩
  grw [←mul_one t, expand_αβψ_as_ψ_αβ_ψ_αβ_ψ hi₁ hi₂]
...
theorem comm_of_α_αβψ_ψ :
  ...
  apply triv_comm_iff_commutes.mpr
  rw [commutatorElement_def]
  grw [←inv_of_αβψ hj, ←inv_of_ψ hk, expr_α_αβψ_as_αβψ_α hi hj, expr_α_ψ_as_ψ_α hi hk,
...
theorem sufficient_conditions_for_commutator_of_βψ_and_α_β2ψ :
  ...
  have aux₁ : ∀ u' : R, {βψ, i, t} * {α, j, u'} = {α, j, u'} * ⁅{α, j, -u'}, {βψ, i, t}⁆ * {βψ, i, t} := by
    intro u'; rw [inv_of_α]; group
  have aux₂ : ∀ u' : R, {βψ, i, t} * {α, j, u'} = ⁅{α, j, u'}, {βψ, i, t}⁆⁻¹ * {α, j, u'} * {βψ, i, t} := by
    intro u'; group
  ...
  have := (generic_commutator_of_α_βψ Rchar hj hi (-u) t).1
  field_simp at this
  have := calc
...
theorem expr_ψ_αβ_as_αβ_αβ2ψ_αβψ_ψ :
  ...
  intro i j hi hj t u
  have := calc
    {αβ2ψ, i + 2 * j, -t * u^2} * {αβψ, i + j, -t * u} = ⁅{αβ, i, -t}, {ψ, j, u}⁆ := by rw [commutator_of_αβ_ψ_b Rchar hi hj]
    _ = ⁅{αβ, i, t}⁻¹, {ψ, j, u}⁆ := by rw [inv_of_αβ]
  grw [this]
  group

theorem id_of_αβψ : id_of_root (R := R) weakB3Large.pres_mk αβψ := by
  intro i hi
  have := doub_of_αβψ Rchar hi 0
  rw [mul_zero] at this
  exact mul_right_eq_self.1 this

theorem id_of_αβ2ψ : id_of_root (R := R) weakB3Large.pres_mk αβ2ψ := by
  intro i hi
  have := doub_of_αβ2ψ Rchar hi 0
  rw [mul_zero] at this
  exact mul_right_eq_self.1 this
...
theorem hom_lift_of_interchange_of_α2β2ψ_a :
  ...
  · rw [hu, mul_zero, mul_zero, zero_mul, id_of_β2ψ, id_of_αβψ Rchar]; group
  rcases eq_or_ne v 0 with hv | hv
...
theorem hom_lift_of_interchange_of_α2β2ψ_b :
  ...
  intro i j k hi hj hk t u v
  rcases eq_or_ne u 0 with hu | hu
  · rw [hu, mul_zero, zero_mul, id_of_βψ, id_of_αβ2ψ Rchar]; group
  rcases eq_or_ne v 0 with hv | hv
...
theorem hom_lift_of_inv_doub_of_β_αβ2ψ :
  ∀ ⦃i j k : ℕ⦄ (hi : i ≤ 1) (hj : j ≤ 1) (hk : k ≤ 1) (t u : R),
  ...
  · have aux₁ := raw_hom_lift_of_inv_doub_of_β_αβ2ψ_a hi hj hk (u / t) t 1
    have expand₁ := @expand_αβ2ψ_as_α_β2ψ_α_β2ψ _ _ Rchar i (j + 2 * k) hi (by linarith) (u / t) t
    have expand₂ := @expand_αβ2ψ_as_α_β2ψ_α_β2ψ _ _ Rchar i (j + 2 * k) hi (by linarith) (-u / t) t
    have eq1 : u / t * t = u := by field_simp
    have eq2 : (-u / t) * t = -u := by field_simp
    have : {αβ2ψ, i + (j + 2 * k), u / t * t} = {αβ2ψ, i + j + 2 * k, u} := by rw [eq1]; group
    rw [this] at expand₁
    have : {αβ2ψ, i + (j + 2 * k), -u / t * t} = {αβ2ψ, i + j + 2 * k, -u} := by rw [eq2]; group
    rw [this] at expand₂
    --rw [expand₁, expand₂]
    sorry
  sorry
