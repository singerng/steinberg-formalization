...
def refl_def (w : GradedPartialChevalleyGroup Φ R) : FreeGroup (GradedChevalleyGenerator Φ R) →* FreeGroup (GradedChevalleyGenerator Φ R) :=
  FreeGroup.lift ((FreeGroup.map refl_of_gen) ∘ w.define)
...
theorem refl_def_of_present (w : GradedPartialChevalleyGroup Φ R) (g : GradedChevalleyGenerator Φ R) (h : g.ζ ∈ w.sys.present_roots) :
  refl_def w (FreeGroup.of g) = FreeGroup.of (refl_of_gen g) := by
  simp only [refl_def, MonoidHom.coe_comp, Function.comp_apply, FreeGroup.lift.of]
  have : w.define g = FreeGroup.of g := by exact w.h_define h
  rw [this]
...
private theorem refl_deg_of_rels_of_trivial_commutator_of_root_pair (ζ η : Φ)
  (h_ζ : ζ ∈ w.sys.present_roots) (h_η : η ∈ w.sys.present_roots)
    : ∀ r ∈ rels_of_trivial_commutator_of_root_pair R (ζ, η),
      refl_def w r ∈ rels_of_trivial_commutator_of_root_pair R (ζ, η) := by
  intro r h
  simp only [rels_of_trivial_commutator_of_root_pair, mem_setOf_eq] at h
  rcases h with ⟨ i, j, hi, hj, t, u, rfl ⟩
  simp only [map_commutatorElement, free_mk]
...
private theorem refl_deg_of_rels_of_single_commutator_of_root_pair
  ...
  (h_ζ : ζ ∈ w.sys.present_roots) (h_η : η ∈ w.sys.present_roots) (h_θ : θ ∈ w.sys.present_roots)
    : ∀ r ∈ rels_of_single_commutator_of_root_pair R ⟨ ζ, η, θ, C, h_height ⟩,
      refl_def w r ∈ rels_of_single_commutator_of_root_pair R ⟨ ζ, η, θ, C, h_height ⟩ := by
  intro r h
  simp only [rels_of_single_commutator_of_root_pair, mem_setOf_eq] at h
  rcases h with ⟨ i, j, hi, hj, t, u, rfl ⟩
  simp only [map_mul, map_commutatorElement, map_inv, free_mk]
  rw [refl_def_of_present w (GradedChevalleyGenerator.mk ζ i hi t) h_ζ]
  rw [refl_def_of_present w (GradedChevalleyGenerator.mk η j hj u) h_η]
...
private theorem refl_deg_of_rels_of_double_commutator_of_root_pair
  ...
  (h_ζ : ζ ∈ w.sys.present_roots) (h_η : η ∈ w.sys.present_roots) (h_θ₁ : θ₁ ∈ w.sys.present_roots) (h_θ₂ : θ₂ ∈ w.sys.present_roots)
    : ∀ r ∈ rels_of_double_commutator_of_root_pair R ⟨ ζ, η, θ₁, θ₂, C₁, C₂, h_height₁, h_height₂ ⟩,
      refl_def w r ∈ rels_of_double_commutator_of_root_pair R ⟨ ζ, η, θ₁, θ₂, C₁, C₂, h_height₁, h_height₂ ⟩ := by
  intro r h
...
private theorem refl_deg_of_rels_of_mixed_commutes_of_root (ζ : Φ) (h_ζ : ζ ∈ w.sys.present_roots):
  ∀ r ∈ rels_of_mixed_commutes_of_root R ζ,
    refl_def w r ∈ rels_of_mixed_commutes_of_root R ζ := by
  intro r h
...
def refl_valid (w : GradedPartialChevalleyGroup Φ R) :=
  (∀ S ∈ w.lifted_rels_sets, ∀ r ∈ S, w.pres_mk (refl_def w r) = 1)

theorem reflect_degree_of_rels {w : GradedPartialChevalleyGroup Φ R} (h' : refl_valid w) :
    refl_def w '' w.all_rels ⊆ Subgroup.normalClosure w.all_rels := by
  ...
  · apply all_rels_to_normal_closure_all_rels
    ...
    suffices refl_def w t ∈ w.trivial_comm_rels by
    ...
    use ζ, η, h_in_pairs
    exact refl_deg_of_rels_of_trivial_commutator_of_root_pair ζ η (w.sys.h_trivial_valid (ζ, η) h_in_pairs).1 (w.sys.h_trivial_valid (ζ, η) h_in_pairs).2 t h_t_in_rels
  · apply all_rels_to_normal_closure_all_rels
    ...
    suffices refl_def w t ∈ w.single_comm_rels by
      simp only [all_rels, sUnion_insert, sUnion_singleton, mem_union, mem_sUnion]
      tauto
  ...
  · apply all_rels_to_normal_closure_all_rels
    ...
    suffices refl_def w t ∈ w.double_comm_rels by
      simp only [all_rels, sUnion_insert, sUnion_singleton, mem_union, mem_sUnion]
      tauto
