...
/- Generators of the group correspond to matrices with a single *monomial* entry above the diagonal. -/
...
def free_mk_of (ζ : Φ) (i : ℕ) (hi : i ≤ height ζ) (t : R) : FreeGroupOnGradedGens Φ R :=
  FreeGroup.of <| GradedGen.mk ζ i hi t

def reflect_degree_of_gen (Φ : Type TΦ) [PosRootSys Φ] (R : Type TR) [Ring R] (g : GradedGen Φ R) : FreeGroupOnGradedGens Φ R :=
  free_mk_of g.ζ (height g.ζ - g.i) (by omega) g.t
