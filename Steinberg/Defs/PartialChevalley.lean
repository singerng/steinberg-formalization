...
open PosRootSys GradedChevalleyGenerator

variable {G : Type TG} [Group G]
         {Φ : Type TΦ} [PosRootSys Φ]
         {R : Type TR} [Ring R]

structure PartialChevalley (Φ : Type TΦ) [PosRootSys Φ] (R : Type TR) [Ring R] where
  mk ::
  -- autogenerated
...
abbrev graded_group (w : PartialChevalley Φ R) :=
  PresentedGroup (PartialChevalley.all_rels w)

def pres_mk (w : PartialChevalley Φ R) : FreeGroup (GradedChevalleyGenerator Φ R) →* graded_group w :=
  PresentedGroup.mk (PartialChevalley.all_rels w)

/-- Mapping between two PartialChevalley graded groups -/
theorem graded_injection (w₁ w₂ : PartialChevalley Φ R)
...
theorem trivial_commutator_helper {w : PartialChevalley Φ R} {ζ η : Φ}
...
theorem single_commutator_helper (w : PartialChevalley Φ R) (ζ η θ : Φ) (C : R)
...
theorem double_commutator_helper (w : PartialChevalley Φ R) (ζ η θ₁ θ₂ : Φ) (C₁ C₂ : R)
  (h_height₁ : height θ₁ = height ζ + height η)
...
theorem mixed_commutes_helper (w : PartialChevalley Φ R)
...
theorem lin_helper (w : PartialChevalley Φ R) {ζ : Φ} (h : ζ ∈ w.lin_roots)
...
theorem lifted_helper (w : PartialChevalley Φ R)
...
theorem def_helper (w : PartialChevalley Φ R)
...
macro "declare_lin_id_inv_thms" w:ident R:term:arg root:term:arg : command => do
  ...
  let cmds ← Syntax.getArgs <$> `(
    ...
    theorem $linOf : lin_of_root(($w $R).pres_mk, $root) :=
      PartialChevalley.lin_helper ($w $R)
...
macro "declare_mixed_comm_thms" w:ident R:term:arg r:term:arg : command => do
  ...
  let cmds ← Syntax.getArgs <$> `(
    ...
    theorem $mixedName : mixed_commutes_of_root ($w $R).pres_mk $r :=
      PartialChevalley.mixed_commutes_helper ($w $R)
        (by unfold $w; simp [trivial_commutator_pairs])
